...
public interface ResultMapper extends Mapper<Result> {
	...
	@Select("select * from lresult where id = #{id}")
	List<Result> selectById(@Param("id") String id);
	
	@Select("select content from lresult where id = #{id} and type = #{type} order by cre_time")
    List<Result> seletByCond(@Param("id") String id, @Param("type") Integer type);

	@Insert("insert into lresult(id,content,cre_time,type) values (#{id},#{content},#{creTime},#{type}) ")
